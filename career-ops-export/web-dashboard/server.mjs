...
// Reads the same files the CLI uses (data/applications.md via tracker.mjs,
// reports/, portals.yml) and serves them as a small single-page app.
...
function runScan() {
  try {
    const out = execFileSync('node', ['scan.mjs'], { cwd: ROOT, encoding: 'utf8', timeout: 120000 });
...
const server = createServer((req, res) => {
  ...
  if (url.pathname === '/api/portals/toggle' && req.method === 'POST') {
    ...
    req.on('end', () => {
      const { section, name, enabled } = JSON.parse(body);
